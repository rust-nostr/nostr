...
#[doc(hidden)]
pub trait IntoAsyncNostrSigner {
    fn into_nostr_signer(self) -> Arc<dyn AsyncNostrSigner>;
}

impl<T> IntoAsyncNostrSigner for T
where
...
impl<T> IntoAsyncNostrSigner for Arc<T>
where
    T: AsyncNostrSigner + 'static,
{
    fn into_nostr_signer(self) -> Arc<dyn AsyncNostrSigner> {
