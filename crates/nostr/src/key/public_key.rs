...
impl PublicKey {
    ...
    pub fn xonly(&self) -> Result<XOnlyPublicKey, Error> {
        ...
        // if self.xonly.is_null() {
        //     // Parse
        //     let public_key: XOnlyPublicKey = XOnlyPublicKey::from_slice(self.as_bytes())?;
        //     let leaked = Box::leak(Box::new(public_key));
        //
        //     // Cast immutable to mutable
        //     let ptr = self as *const _ as *mut Self;
        //
        //     // SAFETY: We cast a mutable reference to a raw pointer and store it.
        //     unsafe {
        //         (*ptr).xonly = leaked as *const XOnlyPublicKey; // Store the raw pointer
        //     }
        // }
        //
        // // SAFETY: At this point, the `xonly` is guaranteed to point to a valid `XOnlyPublicKey`.
        // let public_key: &XOnlyPublicKey = unsafe { &*self.xonly };
        //
        // assert_eq!(self.to_bytes(), public_key.serialize());
        //
        // Ok(*public_key)
