            ...
            (Some(authors), None) => {
                ...
                let mut outbox: HashMap<RelayUrl, BTreeSet<PublicKey>> = self
                    ...
                    .interact(move |store| {
                        ...
                        let mut relays = Self::map_nip65_outbox_relays(store, &authors)?;

                        // Extend with NIP17 relays
                        let nip17 = Self::map_nip17_relays(store, &authors)?;
                        relays.extend(nip17);
            ...
            (None, Some(p_public_keys)) => {
                ...
                let mut inbox: HashMap<RelayUrl, BTreeSet<PublicKey>> = self
                    ...
                    .interact(move |store| {
                        ...
                        let mut relays = Self::map_nip65_inbox_relays(store, &p_public_keys)?;

                        // Extend with NIP17 relays
                        let nip17 = Self::map_nip17_relays(store, &p_public_keys)?;
                        relays.extend(nip17);
                ...
                let mut relays: HashSet<RelayUrl> = self
                    ...
                    .interact(move |store| {
                        ...
                        let mut relays: HashSet<RelayUrl> =
                            Self::get_nip65_relays(store, union.clone(), None)?;

                        // Extend with NIP17 relays
                        let nip17 = Self::get_nip17_relays(store, union)?;
                        relays.extend(nip17);
