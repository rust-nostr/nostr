...
impl<'a> PartialEq for AccessGuardEvent<'a> {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id
    }
}

impl<'a> Eq for AccessGuardEvent<'a> {}

impl<'a> PartialOrd for AccessGuardEvent<'a> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl<'a> Ord for AccessGuardEvent<'a> {
...
impl<'a> AccessGuardEvent<'a> {
    pub fn new(guard: AccessGuard<'a, &'static [u8]>) -> Result<Self, Error> {
        ...
        let event = DatabaseEvent::decode(value)?;
        let id = *event.id();
