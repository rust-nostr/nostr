...
impl AdmitPolicy for AdmissionPolicyMiddleware {
    ...
    fn admit_event<'a>(&'a self, relay_url: &'a RelayUrl, subscription_id: &'a SubscriptionId, event: &'a Event) -> BoxedFuture<'a, Result<AdmitStatus, PolicyError>> {
        Box::pin(async move {
            // Process event in gossip
            if let Some(gossip) = &self.gossip {
                gossip.process(event, Some(relay_url)).await.map_err(PolicyError::backend)?;   
            }
