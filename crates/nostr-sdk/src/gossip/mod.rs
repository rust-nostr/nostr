...
mod tests {
    ...
    async fn setup() -> (GossipWrapper, TempDir) {
        let temp_dir = TempDir::new().expect("Failed to create temp directory");
        let path = temp_dir.path().join("test.db");

        let db = NostrGossipSqlite::open(path).await.unwrap();
        ...
        (GossipWrapper::new(Arc::new(db)), temp_dir)
    }
    ...
    async fn test_break_down_filter() {
        ...
        let (gossip, _temp_dir) = setup().await;
