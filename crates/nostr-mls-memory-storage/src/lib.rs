...
const _: () = assert!(
    ...
    "DEFAULT_CACHE_SIZE must be greater than 0"
);
...
/// ## Thread Safety
///
/// All caches are protected by `parking_lot::RwLock`s, which allow:
...
/// `parking_lot::RwLock` is used for consistent cross-platform behavior and better performance
...
impl NostrMlsMemoryStorage {
    ...
    pub fn with_cache_size(storage_implementation: MemoryStorage, cache_size: usize) -> Self {
        ...
        let size = NonZeroUsize::new(if cache_size > 0 {
            cache_size
        } else {
            DEFAULT_CACHE_SIZE
        })
        .expect("DEFAULT_CACHE_SIZE must be greater than 0");
...
mod tests {
    ...
    fn test_group_cache() {
        ...
        // Try to find a non-existent group
        let not_found = nostr_storage.find_group_by_mls_group_id(&[5, 6, 7, 8]);
    ...
    fn test_group_relays() {
        ...
        let save_result = nostr_storage.save_group_relay(group_relay);
        assert!(save_result.is_ok());

        // Verify we can now retrieve the relay
        ...
        // Try to add a relay to a non-existent group
        let bad_group_relay = GroupRelay {
            mls_group_id: vec![5, 6, 7, 8],
            relay_url: "wss://relay.example.com".parse().unwrap(),
        };

        let err_result = nostr_storage.save_group_relay(bad_group_relay);
        assert!(err_result.is_err());
        match err_result {
            Err(nostr_mls_storage::groups::error::GroupError::NotFound) => (),
    ...
    fn test_welcome_cache() {
        ...
        let event_id_str = "2be17aa3031bdcb006f0fce80c146dea9c1c0268b0af2398bb673365c6444d45";
        let wrapper_id_str = "00000340cb60be5829fbf2712a285f12cf89e5db951c5303b731651f0d71ac1b";
        let event_id = EventId::from_hex(event_id_str).unwrap();
        let wrapper_id = EventId::from_hex(wrapper_id_str).unwrap();

        // Create a public key
        let pubkey_str = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
        ...
        let event = UnsignedEvent {
            id: None,
            pubkey,
            created_at: Timestamp::from(12345),
        ...
        assert_eq!(found_welcome.mls_group_id, vec![1, 2, 3, 4]);

        // Create a processed welcome
        let processed_welcome = ProcessedWelcome {
            welcome_event_id: Some(event_id),
            wrapper_event_id: wrapper_id,
            processed_at: Timestamp::from(12345),
            state: ProcessedWelcomeState::Processed,
            failure_reason: String::new(),
        };

        // Save the processed welcome
        let processed_welcome_result =
            nostr_storage.save_processed_welcome(processed_welcome.clone());
    ...
    fn test_message_cache() {
        ...
        let message_id_str = "2be17aa3031bdcb006f0fce80c146dea9c1c0268b0af2398bb673365c6444d45";
        let wrapper_id_str = "00000340cb60be5829fbf2712a285f12cf89e5db951c5303b731651f0d71ac1b";
        ...
        // Create an UnsignedEvent for the message
        let event = UnsignedEvent {
            id: None,
            pubkey,
            created_at: Timestamp::from(12345),
        ...
        // Add message in group's messages vector (manually for testing)
        let message = Message {
            id: message_id,
            pubkey,
            kind: Kind::Custom(445),
        ...
        // Create additional entry in messages_by_group_cache
        {
            let mut cache = nostr_storage.messages_by_group_cache.write();
            cache.put(mls_group_id.clone(), Arc::new(vec![message.clone()]));
        }
        ...
        let processed_message_result =
            nostr_storage.save_processed_message(processed_message.clone());
    ...
    fn test_with_custom_cache_size() {
        ...
        // Test caches initialized with custom size
        let mls_group_id = vec![1, 2, 3, 4];
        let group = Group {
            mls_group_id,
            nostr_group_id: "test_group_123".to_string(),
            name: "Custom Cache Test".to_string(),
            description: "Testing custom cache size".to_string(),
            admin_pubkeys: vec![],
            last_message_id: None,
            last_message_at: None,
            group_type: GroupType::Group,
        ...
        // Find the group
        let not_found = nostr_storage.find_group_by_nostr_group_id("nonexistent_group");
        assert!(not_found.is_err());

        let found = nostr_storage.find_group_by_nostr_group_id("test_group_123");
        assert!(found.is_ok());
    }
    ...
    fn test_default_implementation() {
        ...
        let group = Group {
            mls_group_id: mls_group_id.clone(),
            nostr_group_id: "test_group_default".to_string(),
            name: "Default Test".to_string(),
            description: "Testing default implementation".to_string(),
            admin_pubkeys: vec![],
            last_message_id: None,
            last_message_at: None,
            group_type: GroupType::Group,
        ...
        // Save the group using the default implementation
        let result = nostr_storage.save_group(group.clone());
        assert!(result.is_ok());
        ...
        let found_group = found_group.unwrap();
        assert_eq!(found_group.nostr_group_id, "test_group_default");
    }
}
