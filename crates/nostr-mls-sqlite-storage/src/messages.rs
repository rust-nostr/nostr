...
impl MessageStorage for NostrMlsSqliteStorage {
    fn save_message(&self, message: Message) -> Result<Message, MessageError> {
        ...
        let tokens_json = serde_json::to_string(&message.tokens).map_err(|e| {
            MessageError::DatabaseError(format!("Failed to serialize tokens: {}", e))
        })?;

        conn_guard.execute(
            "INSERT OR REPLACE INTO messages
             (id, pubkey, kind, mls_group_id, created_at, content, tags, event, wrapper_event_id, tokens)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            params![
                &message.id.to_string(),
                &message.pubkey.to_string(),
                &message.kind.as_u16(),
                ...
                &tags_json,
                &event_json,
                &message.wrapper_event_id.to_string(),
                &tokens_json
            ],
    ...
    fn find_message_by_event_id(&self, event_id: EventId) -> Result<Message, MessageError> {
        ...
        let message = stmt
            .query_row(params![event_id.to_string()], db::row_to_message)
            .map_err(|e| match e {
                rusqlite::Error::QueryReturnedNoRows => MessageError::NotFound,
                _ => MessageError::DatabaseError(e.to_string()),
            })?;
    ...
    fn find_processed_message_by_event_id(
        ...
        let processed_message = stmt
            .query_row(params![event_id.to_string()], db::row_to_processed_message)
            .map_err(|e| match e {
                rusqlite::Error::QueryReturnedNoRows => MessageError::NotFound,
                _ => MessageError::DatabaseError(e.to_string()),
            })?;
    ...
    fn save_processed_message(
        ...
        let state_str: String = processed_message.state.clone().into();
        ...
        conn_guard
            .execute(
             ...
             VALUES (?, ?, ?, ?, ?)",
                params![
                    &processed_message.wrapper_event_id.to_string(),
                    &message_event_id,
