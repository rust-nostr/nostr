...
async fn main() -> Result<()> {
    ...
    let image_nonce = vec![0u8; 12]; // fill in random bytes
    ...
    assert_eq!(
        bob_nostr_mls.get_messages(&bob_mls_group_id).unwrap().len(),
        1,
    ...
    let (charlie_keys, charlie_nostr_mls) = generate_identity();
