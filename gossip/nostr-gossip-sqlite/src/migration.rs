...
mod tests {
    ...
    use super::*;
    use crate::store::NostrGossipSqlite;
    ...
    async fn test_remove_sqlx_migrations() {
        ...
        let db_path: PathBuf = {
            let sqlx_db: &[u8] = include_bytes!("../fixtures/gossip-sqlx.sqlite");
        ...
        // Make sure it's a SQLx database
        {
            ...
            assert!(table_exists(&tx, "public_keys").unwrap());
        }

        // Run migrations
        {
            let _db = NostrGossipSqlite::open(&db_path).await.unwrap();
        }
