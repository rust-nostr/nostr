...
from nostr_sdk import Client, NostrSigner, Keys, Event, UnsignedEvent, Filter, \
    HandleNotification, Timestamp, nip04_decrypt, nip59_extract_rumor, init_logger, LogLevel, Kind, KindEnum, \
                ...
                try:
                    ...
                    if rumor.created_at().as_secs() >= now.as_secs():
                        if rumor.kind().match_enum(KindEnum.SEALED_DIRECT()):
                            ...
                            await client.send_sealed_msg(rumor.author(), f"Echo: {msg}", None)
                        else:
